...
class MusicLibraryFileProcessor {
	...
	/*
	 * Reads in music library file and adds data to collaboration analyzer
	 * Expects first row to be a header and only parses header rows labeled:
	 * "Track Title", "Artist", "Genre" & "Remixer"
	 * Row must have track title and at least one artist or remixer, otherwise row is ignored
    ...
    HashSet<Song> importFile(File file) throws FileNotFoundException {
        ...
        while (headerScanner.hasNext()) {
        	...
        	if (data.compareTo("Track Title") == 0) titleCol = col;
        	if (data.compareTo("Artist") == 0) artistCol = col;
        	if (data.compareTo("Genre") == 0) genreCol = col;
        	if (data.compareTo("Remixer") == 0) remixerCol = col;
        	col++;
        }
        ...
        while (fileScanner.hasNext()) {
            ...
            while (songScanner.hasNext()) {
                ...
                if (col == titleCol) title = data;
                if (col == genreCol) genre = data;
                if (col == artistCol || col == remixerCol) artistAry.addAll(artistParser(data));
                col++;
            }
    ...
    /*
     * Given input artist data string, returns them in array format with leading and trailing whitespaces removed
    ...
    /*
     * Creates new output file and outputs list of songs to .tsv file with header row
     ...
     */
    String exportFile(File outputFile, Set<Song> songSet) throws FileNotFoundException {
    	// Grab absolute file path to return
    	String absoluteFilePath = outputFile.getAbsolutePath();
    	
    	// Create instance of PrintWriter writing out to an "output.txt" file in the current directory
			...
			while (songIterator.hasNext()) {
	        	...
	        	if (artistList != null) {
		        	for (int j = 0; j < artistList.size(); j++) {
		        		if (j > 0) artistString = artistString + ", ";
		        		artistString = artistString + artistList.get(j);
		        	}
	        	}
	        	String outputString = new String();
	        	if (currentSong.getTitle() != null) outputString = outputString + currentSong.getTitle();
	        	outputString = outputString + "\t";
	        	if (artistString != null) outputString = outputString + artistString;
	        	outputString = outputString + "\t";
	        	if (currentSong.getGenre() != null) outputString = outputString + currentSong.getGenre();
	        	outputString = outputString + "\t";
