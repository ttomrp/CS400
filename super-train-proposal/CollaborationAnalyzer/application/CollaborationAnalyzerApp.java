...
/**
 * This class defines the GUI for the Collaboration Analyzer. 
...
public class CollaborationAnalyzerApp extends Application {
    ...
    private String SIMILAR_TRACKS_HELP_TOOLTIP = 
    		"Select an artist in the dropdown to display songs similar to those of that artist.\n"
    		+ "\n"
    		+ "Finds other artists the selected artist has collorated with, and displays those other artists' songs.";
    private String DEGREES_SEPARATION_HELP_TOOLTIP = 
    		"Select two artists in the dropdown to display their degrees of separation,\n"
    ...
    private String SONGS_FOR_ARTIST_HELP_TOOLTIP = 
    		"Select an artist in the dropdown to display that artist's songs.";
    private static CollaborationAnalyzer db;
    private static ObservableList<String> sortedList;
    private static ObservableValue<Number> librarySize;
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        Label librarySizeLabel = createLabel();
        toolbarBox.getChildren().addAll(toolbar, librarySizeLabel);
    ...
    private VBox similarTracks() {
        ...
        artistCol.setCellValueFactory(cellData -> new  ReadOnlyStringWrapper(cellData.getValue().getArtists().toString().replace("[","").replace("]", "")));
        ...
        stVbox.getChildren().add(createArtistSelectComboBoxContainer(cbox, 
        		e -> {
        			populateSimilarTable(table, cbox.getValue());
        			if (cbox.getValue() == null) table.setPlaceholder(new Label("Select artist to view similar tracks"));
        		}
        	));
    ...
    private Label createLabel() {
    	Label librarySizeText = new Label("Libary Size" + librarySize);
        librarySize.addListener(e -> {
        	librarySizeText.setText("Libary Size " + librarySize.getValue());
        	});
        return librarySizeText;
    }
    ...
    private VBox degreesOfSeparation() {
    	...
    	grid.add(createArtistSelectComboBoxContainer(artist1Box, 
    				e -> {
    	...
    	grid.add(createArtistSelectComboBoxContainer(artist2Box, 
					e -> { 
						populateSepTable(table, artist1Box.getValue(), artist2Box.getValue());
						if ((artist1Box.getValue() == null) || (artist2Box.getValue() == null)) {
			    			table.setPlaceholder(new Label("Select two artists to view degrees separation"));
    ...
    private static TableView<Song> degreesSepTable() {
        ...
        numberCol.setCellValueFactory(cellData -> new ReadOnlyObjectWrapper<Number>(table.getItems().indexOf(cellData.getValue()) + 1));
        TableColumn<Song,String> titleCol = new TableColumn<Song,String>("Title");
        titleCol.setCellValueFactory(new PropertyValueFactory<>("title"));
        TableColumn<Song,String> artistCol = new TableColumn<Song,String>("Artists");
        artistCol.setCellValueFactory(cellData -> new  ReadOnlyStringWrapper(cellData.getValue().getArtists().toString().replace("[","").replace("]", "")));
    ...
    /**
     * Populate the degrees of separation table with data. Calling this "refreshes" the table
     * @param table
     * @param name1
     * @param name2
     */
...
//    		table.setPlaceholder(new Label("No connection between artists"));
    ...
    private static void addButtonAction(Stage popupStage,boolean isSingleAction, TextField titleField, TextField artistField, TextField genreField) {
		// retrieve inputs
		...
		artistField.clear();
		genreField.clear();
		if (isSingleAction) popupStage.close();	
    }
    ...
    private static void removeButtonAction(Stage popupStage, boolean isSingleAction, ComboBox<String> artistBox, ComboBox<String> titleBox) {
    	// retrieve inputs
    ...
    private VBox songsForArtist( ) {
        ...
        ComboBox<String> cbox = new ComboBox<String>();
        vbox.getChildren().add(createArtistSelectComboBoxContainer(cbox,
        		e -> populateSongsTable(table, cbox.getValue()) 
        	));
    ...
    /**
     * Creates a combo box wrapped in an HBox, which puts some
     * padding under the combo box
    ...
    private void showAboutPopup() {
    	...
    	Text aboutText = new Text(
    			"Created by Matthias Schmitz, Adam Cook, Felix Lin, Tomas Perez, and Jon McMahon.\n"
